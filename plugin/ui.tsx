...
const WIDTH_LS_KEY = "html-to-figma.widthSetting";
const FRAMES_LS_KEY = "html-to-figma.useFramesSetting";
const EXPERIMENTS_LS_KEY = "html-to-figma.showExperiments";
...
class App extends SafeComponent {
  ...
  @observable urlValue = "https://google.com";
  ...
  async getImageUrl(
    ...
    let hash = imageHash;
    if (!hash) {
      hash = md5.ArrayBuffer.hash(intArr.buffer as ArrayBuffer);
    }
    const fromCache =
      hash &&
      this.clientStorage &&
  ...
  async getCode() {
    ...
    const blocks = selectionToCode(this.selectionWithImages as any);
    ...
    if (newExperimentsUi) {
      this.iframeRef?.contentWindow?.postMessage(
        {
          type: "html-to-figma.draggingInItem",
          data: {
            item: blocks,
          },
        },
      ...
      link.setAttribute("download", "page.json");
  ...
  onCreate = () => {
    ...
    if (this.urlValue) {
      ...
      const apiRoot = this.apiRoot || "http://localhost:4000";
  ...
  importFromServer = async () => {
    ...
    try {
      console.log("Fetching latest capture from handoff server...");
      
      const response = await fetch(`${HANDOFF_SERVER}/api/latest-capture`);
      
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.message || "No capture available");
      }

      const captureData = await response.json();
      console.log(`✅ Received capture #${captureData.id} from ${captureData.url}`);

      const data = captureData.data;
      const layers = data.layers;

      // Process images just like in onCreate
      await Promise.all(
        [data].concat(
          layers.map(async (rootLayer: Node) => {
            await traverseLayers(rootLayer, (layer) => {
      ...
      // Send to Figma
      parent.postMessage(
        { pluginMessage: { type: "import", data: data } },
        "*"
      );
      
      console.log("✅ Successfully imported from server!");
    } catch (err) {
      this.loading = false;
      const message = (err as any).message || String(err);
      
      if (message.includes("Failed to fetch") || message.includes("NetworkError")) {
        this.errorMessage = "Handoff server not running. Please start it with: node handoff-server.js";
      } else if (message.includes("No capture available")) {
        this.errorMessage = "No capture available. Use the Chrome extension to capture a page first.";
      } else {
          ...
          >
            ...
            {this.loading ? (
              ...
              <>
                ...
                >
                  Fetch from Chrome extension via handoff server
                </Typography>
